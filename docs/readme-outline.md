...
| 3 | **Where do my prompts go?** | DOC-02 (D10) | Per-tier data-flow table: Tier 1 local-only, Tier 2 local ONNX, Tier 3 local-only (YAML capability policy, no hosted service) | — |
...
| 5 | **Architecture diagram** | DOC-04 | Mermaid cascade diagram (regex → ONNX → behavioral policy gate) with the ambiguous-band gating | — |
